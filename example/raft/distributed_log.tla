...
Tick ==
  ...
  /\ voted_for' = voted_for
  /\ timers' = [n \in Nodes | IF timers[n] > 0 THEN timers[n] - 1 ELSE 0]
  /\ messages' = messages
...
ClientSubmit(node, cmd) ==
  ...
  /\ msg_id' = msg_id + 1
  /\ current_term' = current_term
  /\ logs' = [logs EXCEPT ![node] = Append(logs[node], [term |-> current_term, command |-> cmd])]
  /\ messages' = messages \/ {
      [type |-> "AppendEntries",
       to |-> n,
       from |-> node,
...
LeaderAppendOnly ==
  \A n \in Nodes: node_state[n] = "leader" => \A i \in 1..(LastLogIndex(logs[n]) - 1): logs[n][i] = logs[n]'[i]
...
EventualLeader == <> (\E n \in Nodes: node_state[n] = "leader")
